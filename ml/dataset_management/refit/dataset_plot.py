...
DATA_DIRECTORY = "/usr/src/tfTest/nilm/ml/dataset_management/refit/"
...
for filename in os.listdir(args.data_dir + appliance_name):
    if (
        dataset == "train"
        and dataset.upper() in filename.upper()
...
for idx, chunk in enumerate(
    pd.read_csv(
        # args.data_dir + appliance_name + "/" + "kettle_training_.csv",
        args.data_dir + appliance_name + "/" + filename,
        # index_col=False,
