...
def transformer(
    window_length=599, drop_out=0.1, d_model=256, **kwargs
) -> tf.keras.Model:
    """Specifies a transformer-based model to be trained in a loop."""
    return NILMTransformerModel(
        window_length=window_length, drop_out=drop_out, hidden=d_model, **kwargs
    )


def transformer_fit(
    window_length=599, drop_out=0.1, threshold=0.5, d_model=256, c0=1.0, **kwargs
) -> tf.keras.Model:
...
def cnn_fun(
    window_length=599, conv_l2=0, dense_l2=0, batch_norm=False, drop_rate=0.3
) -> tf.keras.Model:
    ...
    input_layer_reshape = tf.keras.layers.Reshape(target_shape=(1, window_length, 1))(
        input_layer
    )
    ...
    if batch_norm:
        conv_1 = tf.keras.layers.BatchNormalization(synchronized=True)(
            conv_1
        )  # TODO: synch True causes training to hang after 'tensorflow/compiler/xla/stream_executor/cuda/cuda_dnn.cc:432] Loaded cuDNN version 8600'
    ...
    label_layer = tf.keras.layers.Dense(
        1024, kernel_regularizer=tf.keras.regularizers.L2(dense_l2)
    )(flatten_layer)
    if batch_norm:
    ...
    return tf.keras.models.Model(inputs=input_layer, outputs=output_layer, name="cnn")
...
def fcn(window_length=599, conv_l2=0, batch_norm=False) -> tf.keras.Model:
    ...
    return tf.keras.models.Model(inputs=input_layer, outputs=output_layer, name="fcn")
...
def resnet(window_length, n_feature_maps=16, batch_norm=False) -> tf.keras.Model:
    ...
    conv_x = tf.keras.layers.Conv2D(
        filters=n_feature_maps, kernel_size=(1, 5), padding="same"
    )(input_layer_reshape)
    if batch_norm:
    ...
    conv_y = tf.keras.layers.Conv2D(
        filters=n_feature_maps, kernel_size=(1, 3), padding="same"
    )(conv_x)
    if batch_norm:
    ...
    shortcut_y = tf.keras.layers.Conv2D(
        filters=n_feature_maps, kernel_size=(1, 1), padding="same"
    )(input_layer)
    if batch_norm:
    ...
    conv_x = tf.keras.layers.Conv2D(
        filters=n_feature_maps * 2, kernel_size=(1, 5), padding="same"
    )(output_block_1)
    if batch_norm:
        conv_x = tf.keras.layers.BatchNormalization()(conv_x)
    conv_x = tf.keras.layers.Activation("relu")(conv_x)
    ...
    conv_y = tf.keras.layers.Conv2D(
        filters=n_feature_maps * 2, kernel_size=(1, 3), padding="same"
    )(conv_x)
    if batch_norm:
        conv_y = tf.keras.layers.BatchNormalization()(conv_y)
    conv_y = tf.keras.layers.Activation("relu")(conv_y)
    ...
    # Expand channels for the sum.
    shortcut_y = tf.keras.layers.Conv2D(
        filters=n_feature_maps * 2, kernel_size=(1, 1), padding="same"
    )(output_block_1)
    if batch_norm:
        shortcut_y = tf.keras.layers.BatchNormalization()(shortcut_y)
