...
def get_arguments():
    ...
    parser.add_argument(
        "--crop", type=int, default=None, help="use part of the dataset for testing"
    )
    parser.add_argument(
        "--batch_size", type=int, default=1024, help="sets test batch size"
    )
...
if __name__ == "__main__":
    ...
    test_filename = common.find_test_filename(args.datadir, appliance_name, "test")
    ...
    test_prediction = model.predict(
        x=test_provider, verbose=1, workers=24, use_multiprocessing=True
    )
    ...
    logger.log(
        f"De-normalizing predictions with mean = {app_mean} and std = {app_std}."
    )
    ...
    if args.plot:
        ...
        ax1.plot(
            np.concatenate((pad, ground_truth, pad), axis=None),
            color="#d62728",
            linewidth=1.6,
        )
        ax1.plot(
            np.concatenate((pad, prediction, pad), axis=None),
            color="#1f77b4",
            linewidth=1.5,
        )
        ax1.grid()
        ax1.set_title(
            "Test results on {:}".format(test_filename),
            fontsize=16,
            fontweight="bold",
            y=1.08,
        )
