...
columns.remove(
    params_appliance[appliance_name]["channels"][
        params_appliance[appliance_name]["houses"].index(house)
    ]
)
...
for name in columns_names:
    if name == appliance_name:
        ...
        appliances["{}".format(name)] = df_alias.loc[
...
for idx, layer in enumerate(model.layers):
    ...
    if "conv2d_" in layer.name or "cnn" in layer.name:
        ...
        colormap0 = ax[cnn].imshow(weights_arr, cmap="magma", aspect="auto")
...
if args.transfer:
    fig1.suptitle(
        "{} - CNN layers weigths".format(args.cnn), fontsize=16, fontweight="bold"
    )
else:
    fig1.suptitle(
        "{} - CNN layers weigths".format(args.appliance_name),
        fontsize=16,
        fontweight="bold",
    )
...
# ----------------------------------------------------- Prediction------------------------------------------------------
output = model.predict(inp.reshape(1, -1, length), batch_size=None, steps=None)
...
intermediate_output = intermediate_layer_model.predict(
    inp.reshape(1, -1, length), batch_size=None, steps=None
)
...
inp = inp * 814 + 522
tar = (
    tar * params_appliance[appliance_name]["std"]
    + params_appliance[appliance_name]["mean"]
)
out = (
    output[0][0] * params_appliance[appliance_name]["std"]
    + params_appliance[appliance_name]["mean"]
)
if out < 0:
    out = 0
...
if args.transfer:
    fig3.suptitle(
        "Features from the last (learnt from {:}) CNN layer - (dense {:})".format(
            args.cnn, appliance_name
        ),
        fontsize=16,
        fontweight="bold",
    )
