...
def plot(data, plot_name, plot_display, appliance):
    ...
    plt.xlabel("Epoch")
    plt.legend()
    plot_filepath = os.path.join(args.save_dir, appliance, f"{plot_name}_loss")
    ...
    plt.xlabel("Epoch")
    plot_filepath = os.path.join(args.save_dir, appliance, f"{plot_name}_mae")
    logger.log(f"Plot directory: {plot_filepath}")
    plt.savefig(fname=plot_filepath)
    if plot_display:
        plt.show()
...
def get_arguments():
    ...
    parser.add_argument("--batchsize", type=int, default=1024, help="mini-batch size")
    parser.add_argument(
        "--n_epoch", type=int, default=50, help="Number of epochs to train over."
    )
    parser.add_argument(
        "--crop_train_dataset",
        type=int,
...
if __name__ == "__main__":
    ...
    with strategy.scope():
        ...
        def compute_train_loss(y, y_status, y_pred, y_pred_status, model_losses):
            ...
            mse_loss = tf.reduce_sum(mse_loss_obj(y, y_pred)) * (
                1.0 / global_batch_size
            )
            bce_loss = tf.reduce_sum(bce_loss_obj(y_status, y_pred_status)) * (
                1.0 / global_batch_size
            )
            ...
            if tf.reduce_any(mask):
                ...
                scale_factor = 1.0 / (mask_size * (1.0 * num_replicas))
        ...
        checkpoint_manager = tf.train.CheckpointManager(
            checkpoint, directory=checkpoint_filepath, max_to_keep=3
        )
    ...
    if args.resume_training:
        if checkpoint_manager.latest_checkpoint is None:
            raise FileNotFoundError(
                "Resume training specified but no checkpoints found."
            )
